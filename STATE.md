...
- **Item 3 (workspace ADR-17, world-map pre-answering) — tier-gate LIVE + serving; extended
  ...
  **The tier-routing gate (Fork B) is BUILT + wired + runs END-TO-END** (`Swarm.WorldMap.Coverage`/
  `Gate`, OFF-by-default): Stage-1 structural fail-closed machine + Stage-2 cheap-LLM entailment
  veto + evidence-closed renderer + a NOLINK circuit-breaker (3s). **Follow-on work all landed
  2026-07-05:** consilium latency **#1** (residency `keep_alive=-1` kills the interactive
  cold-load tax; family decorrelated; judge false_supported 0.5→0 — but the earlier −19/−27%
  latency was partly a num_predict=512 truncation artifact, corrected to 4096, warm consilium
